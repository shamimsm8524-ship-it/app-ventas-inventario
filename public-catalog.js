(()=>{
  ...
  ready(()=>{
    ...
    window.card=function(p){
      ...
      const unit=p.unit?p.unit:'Unidades';
      const badge=stock<=0?'<span class="stock-tag out">🚫 Agotado</span>':`<span class="stock-tag in">🟢 ${stock} ${esc(unit)} disponibles</span>`;
      return `<article class="product"><div class="photo">${p.image?`<img src="${p.image}">`:'Sin imagen'}</div><div class="pb"><h3>${esc(p.name)}</h3><div class="meta">${esc(p.category)}${p.barcode?` · ${esc(p.barcode)}`:''}</div><div class="price">${money(p.sellPrice)}</div><div>${badge}</div><div class="actions"><button class="btn secondary" data-edit="${p.id}">Editar</button><button class="btn primary" data-add="${p.id}">${stock<=0?'Sin stock':'Vender'}</button><button class="btn danger" data-delete="${p.id}">Eliminar</button></div></div></article>`;
    };
    ...
    async function payload(){
      return Promise.all(products.map(async p=>({id:String(p.id||''),name:String(p.name||''),category:String(p.category||''),sellPrice:+p.sellPrice||0,stock:+p.stock||0,unit:String(p.unit||'Unidad'),description:String(p.description||''),specifications:specFor(p),image:await compressImage(p.image||'')})));
    }
    ...
    async function syncNow(force=false){
      ...
      syncing=(async()=>{
        ...
        $('catalogPublicLink').value=url;$('catalogSyncState').textContent='Catálogo actualizado · '+rows.length+' producto(s)';
        return publicId;
