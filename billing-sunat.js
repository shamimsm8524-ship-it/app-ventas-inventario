  ...
  ready(async()=>{
    ...
    const section=document.createElement('section');section.id='billing';section.className='view';section.innerHTML=`
      ...
      <div class="billingHero">
        ...
        <span id="billConfigStatus" class="billingStatus">⚠️ Falta configurar el negocio</span>
        <div class="billingConfig billingForm">
          ...
          <button type="button" class="btn secondary" id="billSaveConfig">Guardar configuración de este negocio</button>
        </div>
    ...
    function loadConfigForm(){
      $('billBizRuc').value=config.ruc||'';$('billBizName').value=config.legalName||'';$('billTradeName').value=config.tradeName||'';$('billRegime').value=config.taxRegime||'';$('billAddress').value=config.address||'';$('billInvoiceSeries').value=config.invoiceSeries||'F001';$('billReceiptSeries').value=config.receiptSeries||'B001';renderConfigSummary();
    }
    function renderConfigSummary(){
      const box=$('billIssuerSummary'),status=$('billConfigStatus');if(!box||!status)return;
      box.innerHTML=`<div>RUC<b>${esc(config.ruc||'Sin configurar')}</b></div><div>Razón social<b>${esc(config.legalName||'Sin configurar')}</b></div><div>Régimen<b>${esc(config.taxRegime||'Sin configurar')}</b></div><div>Series<b>${esc(config.invoiceSeries||'F001')} · ${esc(config.receiptSeries||'B001')}</b></div>`;
      const ok=configReady();status.textContent=ok?'✓ Datos del negocio guardados':'⚠️ Falta completar RUC, razón social y régimen';status.classList.toggle('ok',ok);
    }
    ...
    function renderAll(){fillProducts();fillSales();renderCart();renderDocs();renderConfigSummary()}
    ...
    $('billSaveConfig').onclick=()=>{
      ...
      if(next.ruc&&!/^\d{11}$/.test(next.ruc))return alert('El RUC debe tener 11 dígitos.');
      config=next;try{localStorage.setItem(CFG_STORE,JSON.stringify(config))}catch{}renderConfigSummary();window.vareliaToast?.('Configuración de facturación guardada para este negocio.');
    };
