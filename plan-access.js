(()=>{
  ...
  const PLAN_CACHE='varelia_subscription_plan_v3';
  const AMOUNT=28, MONTHS=1;
  const PAYLOADS={
  ...
  style.textContent=`
    ...
    .vareliaPaymentStatus{display:none;margin:14px 0 0;padding:13px 14px;border-radius:14px;border:1px solid #86efac;background:#f0fdf4;color:#166534;font-size:13px;font-weight:900;text-align:center}.vareliaPaymentStatus.show{display:block}
  `;
  document.head.appendChild(style);

  const overlay=document.createElement('div');
  overlay.className='vareliaPlanModal';
  overlay.innerHTML=`<div class="vareliaPlanCard" role="dialog" aria-modal="true"><div class="vareliaPlanHead"><div><h2 id="vareliaPlanTitle">👑 Varelia Premium</h2><p id="vareliaPlanText">Desbloquea herramientas avanzadas para administrar y analizar tu negocio con mayor control.</p></div><button class="vareliaPlanClose" type="button" aria-label="Cerrar">×</button></div><div class="vareliaPlanBenefits"><div class="vareliaPlanBenefit"><div>📊</div><div><b>Reportes y ganancias avanzadas</b><span>Rentabilidad, márgenes y detalle por producto.</span></div></div><div class="vareliaPlanBenefit"><div>📄</div><div><b>Comprobantes avanzados</b><span>PDF, WhatsApp e impresión mejorada.</span></div></div><div class="vareliaPlanBenefit"><div>☁️</div><div><b>Respaldo y funciones en la nube</b><span>Copias de seguridad y recuperación.</span></div></div><div class="vareliaPlanBenefit"><div>👥</div><div><b>Funciones profesionales</b><span>Usuarios/cajeros, clientes y futuras integraciones.</span></div></div></div><div class="vareliaPriceBox"><div><b>Plan Premium</b><small>Duración: 1 mes</small></div><strong>S/ 28</strong></div><div class="vareliaPaymentStatus" id="vareliaPaymentStatus">✓ Comprobante enviado · En revisión</div><button type="button" class="vareliaPlanCta">Comprar Premium · S/ 28</button></div>`;
  document.body.appendChild(overlay);
  const title=overlay.querySelector('#vareliaPlanTitle'),text=overlay.querySelector('#vareliaPlanText'),cta=overlay.querySelector('.vareliaPlanCta'),paymentStatus=overlay.querySelector('#vareliaPaymentStatus');

  const pay=document.createElement('div');
  pay.className='vareliaPayModal';
  pay.innerHTML=`<div class="vareliaPayCard" role="dialog" aria-modal="true"><div class="vareliaPayHead"><div><h2>💳 Pagar Varelia Premium</h2><p>Plan de 1 mes por S/ 28.00. Realiza el pago y adjunta el comprobante.</p></div><button class="vareliaPayClose" type="button" aria-label="Cerrar">×</button></div><div class="vareliaPriceBox"><div><b>Total a pagar</b><small>Premium por 1 mes</small></div><strong>S/ 28</strong></div><div class="vareliaPayTabs"><button type="button" class="vareliaPayTab active" data-pay="yape">Yape</button><button type="button" class="vareliaPayTab" data-pay="plin">Plin</button></div><div class="vareliaQrWrap"><div class="vareliaQrBox" id="vareliaPayQr">Cargando QR…</div><div class="vareliaMerchant" id="vareliaMerchant"></div></div><div class="vareliaPaySteps">1. Escanea el QR con Yape o Plin.<br>2. Paga exactamente <b>S/ 28.00</b>.<br>3. Guarda la captura del pago.<br>4. Adjunta el comprobante aquí para revisión.</div><div class="vareliaReceiptField"><label>Nombre de quien realizó el pago (opcional)</label><input id="vareliaPayerName" maxlength="120" placeholder="Ej.: Milagros Quispe"></div><div class="vareliaReceiptField"><label>Comprobante de pago</label><input id="vareliaReceipt" type="file" accept="image/jpeg,image/png,image/webp,application/pdf"><div class="vareliaFileName" id="vareliaFileName">Selecciona una captura o PDF (máx. 5 MB).</div></div><p class="vareliaPayNote">El Premium se activa cuando el pago sea revisado y aprobado. La vigencia será de 1 mes desde la aprobación.</p><button type="button" class="vareliaPaySubmit">Enviar comprobante</button></div>`;
  ...
  function updateCta(){
    paymentStatus?.classList.toggle('show',paymentSubmitted&&!validPremium());
    if(validPremium()){cta.disabled=true;cta.textContent='✓ Premium activo';return}
    if(paymentSubmitted){cta.disabled=true;cta.textContent='✓ Comprobante enviado · En revisión';return}
    cta.disabled=false;cta.textContent=pendingRequest?'Continuar pago · S/ 28':'Comprar Premium · S/ 28';
  }
  ...
  const showPay=()=>{if(validPremium()||paymentSubmitted){showPremium(paymentSubmitted?'✓ Pago Premium en revisión':'👑 Tus beneficios Premium',paymentSubmitted?'Tu comprobante ya fue enviado y está en revisión. No necesitas enviarlo nuevamente.':'Tu Plan Premium está activo.');return;}closeBenefits();setMethod(paymentMethod);pay.classList.add('show')};cta.onclick=showPay;
  ...
  function applyCard(){
    ...
    else if(state==='review')card.innerHTML='<div>◇ <b>Plan Básico</b></div><small>✓ Comprobante enviado · En revisión</small><a href="#" class="vareliaPlanLink">Ver estado →</a>';
