...
async function enterWithSession(session){if(!session?.user){showAuth();return}try{await ensureAccount(session.user);showApp(session.user)}catch(e){showAuth();msg('No se pudo cargar tu cuenta: '+(e.message||e),'error')}}
...
$('#vareliaLogout').addEventListener('click',async()=>{await sb.auth.signOut();showAuth();msg('Sesión cerrada.','ok')});
