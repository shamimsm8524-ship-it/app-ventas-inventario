(()=>{
  ...
  ready(()=>{
    ...
    btn.type='button';btn.className='dashboardNav';btn.dataset.view='dashboard';btn.textContent='Dashboard';
    nav.insertBefore(btn,nav.firstChild);

    const section=document.createElement('section');
    section.id='dashboard';section.className='view vareliaDashboard';
    section.innerHTML=`
      <div class="dashWelcome"><div><h2>¡Bienvenida, <span id="dashBusinessName">Varelia</span>! 👋</h2><p>Aquí tienes el resumen general de tu negocio.</p></div><div class="dashQuick"><input class="dashSearch" id="dashSearch" placeholder="Buscar productos, ventas..."><button class="btn primary" id="dashNewSale">+ Nueva venta</button></div></div>
      <div class="dashKpis">
      ...
      <div class="dashMainGrid">
        <article class="dashPanel"><div class="dashPanelTitle"><strong>Ventas de los últimos 7 días</strong><span>Datos reales</span></div><div class="dashChart" id="dashChart"></div></article>
      ...
      <div class="dashBanner"><div><strong>Resumen del mes ✨</strong><p>Este panel usa los datos reales que ya existen en Varelia.</p></div><div class="dashBannerMetric" id="dashBannerMetric">S/ 0.00</div></div>`;
    ...
    function data(){
      let ps=[],ss=[],ms=[];
      try{if(Array.isArray(products))ps=products}catch{}
      try{if(Array.isArray(sales))ss=sales}catch{}
      try{if(Array.isArray(movements))ms=movements}catch{}
      return{ps,ss,ms};
    }
    function refreshDashboard(){
      const {ps,ss,ms}=data(),now=new Date(),today=startOfDay(now);
      const todaySales=ss.filter(s=>new Date(s.date)>=today);
      const monthSales=ss.filter(s=>sameMonth(new Date(s.date),now));
      ...
      const max=Math.max(1,...days.map(x=>x.total));dashChart.innerHTML=days.map(x=>`<div class="dashDay"><div class="dashBarWrap"><div class="dashBar" style="height:${Math.max(4,Math.round((x.total/max)*100))}%" title="${money(x.total)}"></div></div><small>${esc(x.label)}</small></div>`).join('');

      const counts={};ps.forEach(p=>{const c=String(p.category||'Sin categoría');counts[c]=(counts[c]||0)+1});const cats=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,4);const totalCats=Math.max(1,cats.reduce((a,x)=>a+x[1],0));const perc=cats.map(x=>x[1]/totalCats*100);const d1=(perc[0]||0).toFixed(1)+'%',d2=((perc[0]||0)+(perc[1]||0)).toFixed(1)+'%',d3=((perc[0]||0)+(perc[1]||0)+(perc[2]||0)).toFixed(1)+'%';dashDonut.style.setProperty('--d1',d1);dashDonut.style.setProperty('--d2',d2);dashDonut.style.setProperty('--d3',d3);dashCatTotal.textContent=`${ps.length} productos`;dashLegend.innerHTML=cats.length?cats.map(([name,count])=>`<div class="dashLegendRow"><span>${esc(name)}</span><b>${count}</b></div>`).join(''):'<div class="dashEmpty">Sin categorías todavía.</div>';
    ...
    document.addEventListener('click',()=>setTimeout(refreshDashboard,120));
    document.addEventListener('change',()=>setTimeout(refreshDashboard,120));
