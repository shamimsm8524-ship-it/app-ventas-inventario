(()=>{
  ...
  style.textContent=`
    .vpAdminNav{position:relative}.vpAdminBadge{float:right;min-width:20px;height:20px;padding:0 6px;border-radius:999px;background:#ec4899;color:#fff;font-size:10px;display:inline-grid;place-items:center;margin-left:8px}
  ...
  let navBtn=null,isAdmin=false,rows=[];
  ...
  function addNav(){
    ...
    navBtn=document.createElement('button');navBtn.type='button';navBtn.className='vpAdminNav';navBtn.innerHTML='👑 Solicitudes Premium <span class="vpAdminBadge" hidden>0</span>';nav.appendChild(navBtn);navBtn.onclick=()=>{modal.classList.add('show');loadRequests()};
  }
  ...
  function render(){
    ...
    list.innerHTML=rows.map(r=>{
      ...
      return `<div class="vpReq" data-id="${esc(r.id)}"><div class="vpReqTop"><div><h3>${esc(r.business_name||'Negocio')}</h3><div class="vpReqEmail">${esc(r.user_email||'')}</div></div><span class="vpReqStatus ${esc(r.status)}">${statusLabel(r.status)}</span></div><div class="vpReqGrid"><div class="vpReqMini"><b>Plan</b>S/ ${Number(r.amount||20).toFixed(2)} · ${Number(r.duration_months||1)} mes</div><div class="vpReqMini"><b>Método</b>${esc((r.payment_method||'—').toUpperCase())}</div><div class="vpReqMini"><b>Pagador</b>${esc(r.payer_name||'No indicado')}</div><div class="vpReqMini"><b>Enviado</b>${dateFmt(r.paid_at||r.created_at)}</div></div><div class="vpReqActions">${hasReceipt?'<button type="button" class="vpViewReceipt">👁 Ver comprobante</button>':''}${pending&&hasReceipt?'<button type="button" class="vpApprove">✓ Aprobar Premium</button><button type="button" class="vpReject">✕ Rechazar</button>':''}</div></div>`
    }).join('');
  }
  async function loadRequests(){
    if(!isAdmin||!window.vareliaSupabase)return;list.classList.add('vpAdminLoading');
    try{const {data,error}=await window.vareliaSupabase.rpc('get_premium_requests_admin');if(error)throw error;rows=Array.isArray(data)?data:[];render()}
    catch(e){console.error('Premium admin',e);list.innerHTML='<div class="vpAdminEmpty">No se pudieron cargar las solicitudes.</div>'}
  ...
  async function detectAdmin(){
    ...
    try{const {data:userData}=await sb.auth.getUser();const uid=userData?.user?.id;if(!uid)return false;const {data,error}=await sb.from('platform_admins').select('user_id').eq('user_id',uid).maybeSingle();if(error)throw error;isAdmin=!!data;if(isAdmin){addNav();loadRequests()}return isAdmin}catch(e){console.warn('Admin Premium',e);return false}
  }
